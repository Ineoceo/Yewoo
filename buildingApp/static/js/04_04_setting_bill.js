...
function create()
{
	...
	var html = '<table class="table table-bordered table-condensed table-center no-margin" style="text-align:center">';
	html += '<colgroup>';
	html += '<col width="15%">';
	html += '<col width="20%">';
	html += '<col width="5%">';
	...
	for (var i = 0; i < Number(total); i++)
	{
		...
		html += '<td><input id="create'+i+'_sdate" type="text" class="margin0 form-control input-sm" value="'+sy+'.'+sm+'.'+temp_sd+'" onclick="show_datepicker('+"'"+i+"', 's'"+');" onchange="change_date('+"'"+i+"', 's'"+');" disabled /></td>';
		html += '<td>~</td>';
		html += '<td><input id="create'+i+'_edate" type="text" class="margin0 form-control input-sm" value="'+ey+'.'+em+'.'+temp_ed+'" onclick="show_datepicker('+"'"+i+"', 'e'"+');" onchange="change_date('+"'"+i+"', 'e'"+');" disabled /></td>';
		html += '<td>';
		html += '<button type="button" id="create'+i+'_0_m" class="btn btn-sm btn-primary" onclick="create_modify0('+"'"+i+"', 'm'"+');">수정</button>';
		html += '<button type="button" id="create'+i+'_0_d" class="btn btn-sm btn-success" style="display:none" onclick="create_modify0('+"'"+i+"', 'd'"+');">완료</button>';
		html += '</td>';
		html += '<td><input id="create'+i+'_ndate" type="text" class="margin0 form-control input-sm" value="'+ey+'.'+em+'.'+temp_ed+'" onclick="show_datepicker('+"'"+i+"', 'n'"+');" onchange="change_date('+"'"+i+"', 'n'"+');" disabled /></td>';
		html += '<td>';
		html += '<button type="button" id="create'+i+'_1_m" class="btn btn-sm btn-primary" onclick="create_modify1('+"'"+i+"', 'm'"+');">수정</button>';
		html += '<button type="button" id="create'+i+'_1_d" class="btn btn-sm btn-success" style="display:none" onclick="create_modify1('+"'"+i+"', 'd'"+');">완료</button>';
		html += '</td>';
		html += '</tr>';
	...
	html += '</tbody>';
	html += '</table>';
	html += '<div class="col-md-12" style="padding:0px; margin:10px 0px 0px 0px; text-align:center">';
	html += '<button type="button" class="btn btn-sm btn-primary" onclick="create_done();">완료</button>';
	html += '<button type="button" class="btn btn-sm btn-default" onclick="cancel();">취소</button>';
	html += '</div>';
