...
function addItem()
{
	...
	html += '<td width="8%" style="text-align:center">개별안내</td>';
	...
	html += '<td width="8%"><input type="text" class="margin0 form-control input-sm date_-'+lastItemNo+'" value="' + $('#add_date').val().trim() + '" /></td>';
	html += '<td width="5%" style="text-align:center">분류</td>';
	var category = '';
	html += '<td width="10%">';
	html += '<select id="sel_item_-'+lastItemNo+'" class="margin0 form-control input-sm">';
	html += '<option value="1">기타사항</option>';
	html += '</select></td>';
	html += '<td width="34%"><input type="text" class="margin0 form-control input-sm memo_-'+lastItemNo+'" value="' + $('#add_memo').val().trim() + '" /></td>';
	html += '<td width="12%" style="text-align:center">';
	html += '<div class="modifying form-inline"><button type="button" class="btn btn-sm btn-primary">수정</button><button type="button" class="btn btn-sm btn-danger" onclick="del(' + "'-" + lastItemNo + "'" + ');">삭제</button></div>';
	html += '</td>';
	html += '</tr>';
