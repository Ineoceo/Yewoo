...
function filter_leave(f) // label (ㄱ,ㄴ,ㄷ,...) 클릭했을 때 검사하는 함수
{
	// label 색 선택
	for (i = 0; i <= 14; i++)
		$('#label'+i).removeClass('label-info').removeClass('label-primary');
	...
	if (f == 'all') {
		$('#labelall').removeClass('label-info');
		$('#labelall').addClass('label-primary');
