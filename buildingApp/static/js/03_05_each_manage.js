...
function addItem(bid, cnt, yymm)
{
	...
	html += '<td width="8%" style="text-align:center">입력담당자</td>';
	html += '<td width="8%" style="text-align:center" class="name_-'+lastItemNo+'_'+yymm+'">' + $('#add_name_'+yymm).val().trim() + '</td>';
	html += '<td width="8%"><input type="text" class="margin0 form-control input-sm date_-'+lastItemNo+'_'+yymm+'" value="' + $('#add_date_'+yymm).val().trim() + '" /></td>';
	html += '<td width="5%" style="text-align:center">분류</td>';
	var category = '';
	html += '<td width="10%">';
	html += '<select id="sel_item_-'+lastItemNo+'_'+yymm+'" class="margin0 form-control input-sm">';
	html += '<option value="1">기타사항</option>';
	html += '</select></td>';
	html += '<td width="37%"><input type="text" class="margin0 form-control input-sm memo_-'+lastItemNo+'_'+yymm+'" value="' + $('#add_memo_'+yymm).val().trim() + '" /></td>';
	html += '<td width="12%" style="text-align:center">';
	html += '<div class="modifying_'+yymm+' form-inline"><button type="button" class="btn btn-sm btn-primary">수정</button><button type="button" class="btn btn-sm btn-danger" onclick="del('+"'-"+lastItemNo+'_'+yymm+"', '" + yymm + "'" + ');">삭제</button></div>';
	html += '</td>';
	html += '</tr>';
...
var doAjax_Done = function() {
	...
	$.ajax({
		type : 'POST',
		url : '/lease/bill/each/manage/confirm/',
		data : post,
